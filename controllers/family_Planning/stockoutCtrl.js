    ...
    .controller("stockoutCtrl",function ($rootScope,$scope,$http,$location,$timeout,olData,olHelpers,shared,portalService,FPManager) {
        ...
        $scope.getSelectedValues = function(){
            ...
            }else
            {
                ...
                $scope.StockOutObject.chartObject.options.title.text ="Percent of Facilities Stocked out of Pills or Injectables "+FPManager.getlastTwelveMonthName($scope.data.selectedMonth);
                ...
                $.post( portalService.base + "dhis-web-commons-security/login.action?authOnly=true", {
                    ...
                    console.log(FPManager.lastTwelveMonthForSql($scope.data.selectedMonth))
                    FPManager.getFPFacilityList().then(function(data){
                        $scope.StockOutObject.loadingMessage = "Fetching Stock Out Data...";
                        $http.get(portalService.base+'api/sqlViews/Fvxf4sjmWxC/data.json?'+FPManager.lastTwelveMonthForSql($scope.data.selectedMonth)).success(function(val1){
                            $rootScope.showProgressMessage = false;
