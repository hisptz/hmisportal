                ...
                $.post(portalService.base + "dhis-web-commons-security/login.action?authOnly=true", {
                    ...
                    FPManager.getFPFacilityList().then(function (data) {
                        $http.get(portalService.base + 'api/sqlViews/ahVxVlhDa82/data.json?var=year:' + FPManager.lastMonthWithData).success(function (facilities) {
                            ...
                            angular.forEach(nsvRegions, function (value, key) {
                                nsvData.push({
                                    name: key,
                                    value: parseFloat($scope.getNumberPerOu2(data.organisationUnits, value, facilities.rows, value, 'NSV', key))
                                })
                            });
                            angular.forEach(miniLapRegions, function (value, key) {
                                miniLapData.push({
                                    name: key,
                                    value: parseFloat($scope.getNumberPerOu2(data.organisationUnits, value, facilities.rows, value, 'Mini Lap'))
                                })
                            });
                            angular.forEach(iucdRegions, function (value, key) {
                                iucdData.push({
                                    name: key,
                                    value: parseFloat($scope.getNumberPerOu2(data.organisationUnits, value, facilities.rows, value, 'IUCDs'))
                                })
                            });
                            angular.forEach(shortActingRegions, function (value, key) {
                                shortActingData.push({
                                    name: key,
                                    value: parseFloat($scope.getNumberPerOu2(data.organisationUnits, value, facilities.rows, value, 'Short Acting'))
                                })
                            });
                            angular.forEach(implantRegions, function (value, key) {
                                implantData.push({
                                    name: key,
                                    value: parseFloat($scope.getNumberPerOu2(data.organisationUnits, value, facilities.rows, value, 'Implants'))
                                })
                            });
                            ...
                            $scope.orgunitsWithLowTraining = [
                                {
                                    name: 'Short Acting',
                                    region1: shortActingData[0].name + "( " + shortActingData[0].value + "% )",
                                    region2: shortActingData[1].name + "( " + shortActingData[1].value + "% )",
                                    region3: shortActingData[2].name + "( " + shortActingData[2].value + "% )"
                                },
                                {
                                    name: 'IUCD',
                                    region1: iucdData[0].name + "( " + iucdData[0].value + "% )",
                                    region2: iucdData[1].name + "( " + iucdData[1].value + "% )",
                                    region3: iucdData[2].name + "( " + iucdData[2].value + "% )"
                                },
                                {
                                    name: 'Implant',
                                    region1: implantData[0].name + "( " + implantData[0].value + "% )",
                                    region2: implantData[1].name + "( " + implantData[1].value + "% )",
                                    region3: implantData[2].name + "( " + implantData[2].value + "% )"
                                },
                                {
                                    name: 'Mini-lap',
                                    region1: miniLapData[0].name + "( " + miniLapData[0].value + "% )",
                                    region2: miniLapData[1].name + "( " + miniLapData[1].value + "% )",
                                    region3: miniLapData[2].name + "( " + miniLapData[2].value + "% )"
                                },
                                {
                                    name: 'NSV',
                                    region1: nsvData[0].name + "( " + nsvData[0].value + "% )",
                                    region2: nsvData[1].name + "( " + nsvData[1].value + "% )",
                                    region3: nsvData[2].name + "( " + nsvData[2].value + "% )"
                                }
                            ];
                            $timeout(function () {
